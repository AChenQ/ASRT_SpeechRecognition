...
from keras.layers import (Activation, BatchNormalization, Conv2D, Dense,  # , Flatten; , Merge
                          Dropout, Input, Lambda, MaxPooling2D, Reshape, TimeDistributed)
...
class ModelSpeech:  # 语音模型类
    ...
    def CreateModel(self):
        ...
        input_data = Input(
            name="the_input", shape=(self.AUDIO_LENGTH, self.AUDIO_FEATURE_LENGTH, 1)
        )
        ...
        layer_h17 = Dense(128, activation="relu", use_bias=True, kernel_initializer="he_normal")(
            layer_h16
        )  # 全连接层
        ...
        for epoch in range(epoch):  # 迭代轮数
            ...
            while True:
                try:
                    ...
                    self._model.fit_generator(yielddatas, save_step, epochs=epoch)
                    n_step += 1
    ...
    def LoadModel(
        self,
        filename=abspath + "model_speech/m" + ModelName + "/speech_model" + ModelName + ".model",
    ):
        """
                加载模型参数
                """
    ...
    def TestModel(
        ...
        try:
            ...
            if out_report == True:
                txt_obj = open(
                    "Test_Report_" + str_dataset + "_" + nowtime + ".txt", "w", encoding="UTF-8"
                )  # 打开文件并读入
            ...
            print(
                "*[Test Result] Speech Recognition " + str_dataset + " set word error ratio: ",
